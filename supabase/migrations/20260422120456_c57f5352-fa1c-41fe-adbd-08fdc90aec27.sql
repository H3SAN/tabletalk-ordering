-- Direct owner read policy on restaurants (independent of staff_users membership)
DROP POLICY IF EXISTS "owner view restaurant" ON public.restaurants;
CREATE POLICY "owner view restaurant"
ON public.restaurants
FOR SELECT
USING (auth.uid() = owner_user_id);

-- Atomic restaurant workspace bootstrap
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.restaurants WHERE owner_user_id = _uid) THEN
    RAISE EXCEPTION 'You already own a restaurant' USING ERRCODE = '23505';
  END IF;

  -- Slug uniqueness
  IF EXISTS (SELECT 1 FROM public.restaurants WHERE slug = _normalized_slug) THEN
    RAISE EXCEPTION 'That slug is already taken' USING ERRCODE = '23505';
  END IF;

  -- Get email + display name from auth.users
  ...
  RETURN QUERY SELECT _r_id, _b_id, _normalized_slug;
END;
$$;

-- Restrict execution to authenticated users
REVOKE ALL ON FUNCTION public.create_restaurant_workspace(text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_restaurant_workspace(text, text, text, text, text) TO authenticated;

-- Convenience: deterministic restaurant context for the signed-in user
...
REVOKE ALL ON FUNCTION public.get_my_restaurant_context() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_my_restaurant_context() TO authenticated;